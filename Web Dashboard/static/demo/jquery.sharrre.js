 ...
 */ (function ($, window, document, undefined) {
    ...
    loadButton = {
      ...
      delicious: function (self) {
        if (self.options.buttons.delicious.size == "tall") {
          var css = "width:50px;",
            cssCount =
              "height:35px;width:50px;font-size:15px;line-height:35px;",
      ...
      digg: function (opt) {
        window.open(
          "http://digg.com/tools/diggthis/submit?url=" +
            encodeURIComponent(
              opt.buttons.digg.url !== "" ? opt.buttons.digg.url : opt.url
            ) +
            "&title=" +
            opt.text +
  ...
  Plugin.prototype.init = function () {
    ...
    if (this.options.urlCurl !== "") {
      ...
      urlJson.stumbleupon =
        this.options.urlCurl + "?url={url}&type=stumbleupon";
    }
    ...
    $(this.element).hover(
      function () {
        if (
          $(this).find(".buttons").length === 0 &&
          self.options.enableHover === true
        ) {
          self.loadButtons();
        }
  ...
  Plugin.prototype.getSocialJson = function (name) {
    var self = this,
      count = 0,
      url = urlJson[name].replace(
        "{url}",
        encodeURIComponent(this.options.url)
      );
    if (
      this.options.buttons[name].urlCount === true &&
    ...
    if (url != "" && self.options.urlCurl !== "") {
      $.getJSON(url, function (json) {
        if (typeof json.count !== "undefined") {
          ...
          json.data &&
          json.data.length > 0 &&
          typeof json.data[0].total_count !== "undefined"
        ) {
